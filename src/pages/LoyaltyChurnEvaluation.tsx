...
const LoyaltyChurnEvaluation = () => {
  const [latest, setLatest] = useState<ChurnSnapshot | null>(null);
  const [history, setHistory] = useState<ChurnSnapshot[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      loyaltyBiEvaluationService.getChurnLatest(),
      loyaltyBiEvaluationService.getChurnHistory(),
    ])
      ...
      .catch((e) => {
        console.error(e);
        if (cancelled) return;
        setError('خطا در دریافت ارزیابی ریزش مشتریان');
      })
      .finally(() => {
        if (cancelled) return;
        setLoading(false);
  ...
  return (
    <div>
      <h1 className="text-3xl font-serif text-zafting-accent mb-6">
        ارزیابی ریزش مشتریان — باشگاه مشتریان
      </h1>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white/60 p-6 rounded-xl shadow-sm border border-zafting-accent/10">
              <h3 className="text-lg font-medium text-zafting-text mb-2">نرخ ریزش فعلی</h3>
              <p className="text-3xl font-bold text-red-600">
                {latest ? `${latest.churnRatePercent}%` : '-'}
              </p>
            ...
            <div className="bg-white/60 p-6 rounded-xl shadow-sm border border-zafting-accent/10">
              <h3 className="text-lg font-medium text-zafting-text mb-2">در معرض ریزش</h3>
              <p className="text-3xl font-bold text-orange-600">
                {formatNumber(latest?.atRiskCount)}
              </p>
            </div>

            <div className="bg-white/60 p-6 rounded-xl shadow-sm border border-zafting-accent/10">
              <h3 className="text-lg font-medium text-zafting-text mb-2">از دست رفته</h3>
              <p className="text-3xl font-bold text-red-600">
                {formatNumber(latest?.lostCount)}
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 bg-white/60 rounded-xl shadow-sm border border-zafting-accent/10 overflow-hidden">
            <div className="p-5 border-b border-zafting-accent/10">
              <h2 className="text-lg font-bold text-zafting-text">روند نرخ ریزش</h2>
            </div>
            <div className="p-5">
              {history.length === 0 ? (
                ...
                <div className="h-72" dir="ltr">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={history}>
                      ...
                      />
                      <Tooltip
                        formatter={(value: number) => [`${value}%`, 'نرخ ریزش']}
...
export default LoyaltyChurnEvaluation;
